...
if is_gradio_available():
    import gradio as gr
from ...extras.constants import SUPPORTED_MODEL,TP_SUPPORTED_MODEL,CP_SUPPORTED_MODEL,PP_SUPPORTED_MODEL
...
def build_train_config_tab(tabs: gr.Tabs, status_indicator: gr.HTML) -> None:
    with gr.Column():
        ...
        with gr.Row():
            load_dir = gr.Textbox(
                label="模型转换后权重路径",
                value="D:/models_mcore_weights/your_model/",
                interactive=True
            )
            save_dir = gr.Textbox(
                label="模型训练后权重保存路径",
                value="D:/ckpt/your_model",
                interactive=True
            )
        with gr.Row():
            data_path = gr.Textbox(
                label="数据集转换后路径",
                value="D:/dataset/dst/your_dataset_text_document",
                interactive=True
            )
            tokenizer_path = gr.Textbox(
                label="数据集转换后路径",
                value="D:/model_from_hf/your_model/",
                interactive=True
            )
